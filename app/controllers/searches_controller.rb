class SearchesController < ApplicationController
  ...
  def create
    ...
    if params[:tag].nil? == false
      @twitter_results = twitter_search.first_query(:tag => params[:tag], :count =>15, :max_id => params[:twitter_last_id]).results 
      ...
      @twitter_results = twitter_search.first_query(:tag => params[:search], :count =>15).results
